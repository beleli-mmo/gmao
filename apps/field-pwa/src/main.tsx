...
import { startSync, stopSync } from './db/pouch';
import type { ResolvedEquipment } from './lib/qr';
import './app.css';

const API = (import.meta.env.VITE_API_URL as string)?.replace(/\/$/, '') || '';
type Session = { username: string; token: string; fullName: string };
type Screen = { name: 'home' } | { name: 'scan' } | { name: 'ticket'; qrPayload?: string; equipmentId?: string };
...
function App() {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        ...
        setSession(s);
        startSync(s).catch(console.error);
      } catch {
  ...
  if (!session) return <Login onDone={onLogin} />;
  ...
  return (
    <main className="app-home">
      ...
      <button className="app-cta" onClick={() => setScreen({ name: 'ticket' })}>✏️ Créer une DI sans scan</button>
